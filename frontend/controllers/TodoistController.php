			...
			'access' => [
				...
				'rules' => [
					...
					[
    					...
    					'allow' => true,
<<<<<<< HEAD
    					'roles' => ['shop', 'zakup', 'master', 'design', 'program', 'courier', 'system'],
=======
...
<<<<<<< HEAD
                    [
                        'actions' => ['create-modzakaz'],
                        'allow' => true,
                        'roles' => ['admin', 'shop', 'master', 'design'],
                    ],
=======
>>>>>>> origin/master
					[
...
<<<<<<< HEAD
    public function actionCreateModzakaz($id)
    {
        $model = $id;
        $todoist = new Todoist();
        $telegram = new Telegram();
        ...
        if ($todoist->load(Yii::$app->request->post()) && $todoist->validate()) {
            $todoist->save();//сохранение задачи
            if (!$todoist->save()) {
                print_r($todoist->getErrors());
            } else{
                if(Yii::$app->request->isAjax) {
                        /** @var $model \app\models\Zakaz */
                        /*Yii::$app->session->setFlash('update', 'Напоминание успешно создано');*/
                    $notification->getByIdNotificationTodoList('createorder', $todoist);
                        $notification->getSaveNotification();
                        /*$telegram->message($model->id_user, 'Задача была поставлена "'.$model->comment.'" по заказу '.$model->idZakaz->prefics);*/
                       /* $this->findView();*/
                }
        ...
        return $this->renderAjax('create-modzakaz', [
            'model' => $model,
            'todoist'=>$todoist
        ]);
    }
